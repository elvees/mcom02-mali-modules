...
MALI_DRIVER_VERSION ?= r3p0-04rel0
...
build:
	...
	$(MAKE) -C $(MALI_DRIVER_VERSION)/driver/src/devicedrv/mali

install: build
	$(MAKE) -C $(UMP_DRIVER_VERSION)/driver/src/devicedrv/ump install
	$(MAKE) -C $(MALI_DRIVER_VERSION)/driver/src/devicedrv/mali install

clean:
	$(MAKE) -C $(UMP_DRIVER_VERSION)/driver/src/devicedrv/ump clean
	$(MAKE) -C $(MALI_DRIVER_VERSION)/driver/src/devicedrv/mali clean
help:
	echo 'Variables:'
	echo '  KDIR - Path to Linux kernel source directory'
	echo '  UMP_DRIVER_VERSION - UMP driver version, r8p1-00rel0 by default'
	echo '  MALI_DRIVER_VERSION - Mali driver version, r3p0-04rel0 by default'
	echo
	echo 'Goals:'
	echo
